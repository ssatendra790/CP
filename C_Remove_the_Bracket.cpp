    ...
    while (t--)
    {
        ...
        for (i = 2; i < n - 1; i++)
        {
            if (a[i] >= s)
            {
                ...
                {
                    if(F_ckU[i-1][0]+a[i-1]*s > F_ckU[i-1][1]){
                        F_ckU[i][0] = F_ckU[i-1][1];
                    }
                    F_ckU[i][0] = min(F_ckU[i - 1][0] + a[i - 1] * s, F_ckU[i - 1][1]);
                    F_ckU[i][1] = min(F_ckU[i - 1][0] + a[i - 1] * (a[i] - s), F_ckU[i - 1][1]);
                }
            }
            else
            {
                if (a[i - 1] >= s)
                {
                    if(F_ckU[i-1][0]>F_ckU[i-1][1]){
                           F_ckU[i][0] = F_ckU[i-1][1];
                    }
                    F_ckU[i][0] = min(F_ckU[i - 1][0], F_ckU[i - 1][1]);
                    F_ckU[i][1] = min(F_ckU[i - 1][0] + (a[i - 1] - s) * a[i], F_ckU[i - 1][1] + s * a[i]);
                }
                else
                {
                    F_ckU[i][0] = min(F_ckU[i - 1][0], F_ckU[i - 1][1]);
                    F_ckU[i][1] = min(F_ckU[i - 1][0] + a[i - 1] * a[i], F_ckU[i - 1][1]);
                }
            }
        }
        ll ans = 0;
