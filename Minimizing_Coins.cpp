...
void stndr(){
    ll n, i, sum;
    cin >> n >> sum;
    vector<ll>a(n+1),prev(sum+1,1e9);
    for (i = 1; i <= n; i++) cin >> a[i];
    prev[0] = 0;
    for(i=1;i<=n;i++){
        vector<ll>curr(sum+1,1e9);
        curr[0] = 0;
        for(ll j=1;j<=sum;j++){
            curr[j] = prev[j];
            if(j>=a[i]) curr[j] = min(curr[j],1+prev[j-a[i]]);
            if(j>=a[i]) curr[j] = min(curr[j],1+curr[j-a[i]]);
        }
        prev = curr;
    }
    if(prev[sum] >= 1e9) cout<<-1;
    else cout<<prev[sum];
}  
